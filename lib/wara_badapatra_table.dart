...
void didUpdateWidget(WaraBadapatraTable oldWidget) {
  ...
  if (oldWidget.searchCode != widget.searchCode) {
    _autoScrollTimer?.cancel();
    _autoScrollTimer = null;

    // Save current scroll position before restarting
    final double savedPosition = _vController.hasClients 
        ? _vController.offset 
        : 0.0;

    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      // Restore position instead of jumping to 0
      if (_vController.hasClients && savedPosition > 0) {
        _vController.jumpTo(savedPosition);
      }
      // Start scroll from saved position
      _startAutoScrollFromPosition(savedPosition);
    });
  }
}
  ...
  Future.delayed(const Duration(milliseconds: 300), () {
    ...
    // Restore the saved position first
    if (fromPosition > 0 && _vController.hasClients) {
      _vController.jumpTo(fromPosition);
    }
  ...
  // changes made for the TV
  void _startAutoScroll() {
  _autoScrollTimer?.cancel();
  _autoScrollTimer = null;

  // Jump to top first before starting scroll
  if (_vController.hasClients) {
    _vController.jumpTo(0);
  }
  ...
  Widget build(BuildContext context) {
    ...
    : snapshot.data!.where((s) {
        String serviceCode = s.code
            .replaceAll('.', '')
            .trim();
        String nepaliSearch = toNepaliNumber(widget.searchCode)
            .replaceAll('.', '')
            .trim();
        String engSearch = widget.searchCode
            .replaceAll('.', '')
            .trim();

        return serviceCode == nepaliSearch ||
               serviceCode == engSearch;
      }).toList();
...
WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  // Start scroll when all items shown (search cleared) OR enough items
  if (widget.searchCode.isEmpty && filtered.length > 2) {
    _startAutoScroll();
  } else if (widget.searchCode.isNotEmpty && filtered.length > 2) {
    _startAutoScroll();
  }
  // If filtered.length <= 2, scroll stays stopped (correct — 1 result no need to scroll)
});
