...
class _CustomSearchBarState extends State<CustomSearchBar> {
  ...
  bool _handleKeyPress(KeyEvent event) {
    // Only process KeyDown events to prevent double firing (KeyDown + KeyUp)
    if (event is! KeyDownEvent) return false;

    final isTV = Responsive.isTV(context);

    // 1. Handle Asterisk (*) Key - ALWAYS GAINS FOCUS
    if (event.logicalKey == LogicalKeyboardKey.numpadMultiply ||
      ...
      // Force the search bar to gain focus
      if (!widget.focusNode.hasFocus) {
        widget.focusNode.requestFocus();
      }
      return true;
    ...
    // 2. Handle Numeric Keys (Only intercept manually if it's a TV to avoid breaking mobile)
    if (isTV) {
      String? typedDigit;

      // Check if it's a standard character input
      if (event.character != null && RegExp(r'^[0-9]$').hasMatch(event.character!)) {
        typedDigit = event.character;
      } 
      // Fallback for Android TV Remotes that send LogicalKeys instead of characters
      else if (event.logicalKey.keyId >= LogicalKeyboardKey.digit0.keyId &&
               event.logicalKey.keyId <= LogicalKeyboardKey.digit9.keyId) {
        typedDigit = (event.logicalKey.keyId - LogicalKeyboardKey.digit0.keyId).toString();
      } 
      else if (event.logicalKey.keyId >= LogicalKeyboardKey.numpad0.keyId &&
               event.logicalKey.keyId <= LogicalKeyboardKey.numpad9.keyId) {
        typedDigit = (event.logicalKey.keyId - LogicalKeyboardKey.numpad0.keyId).toString();
      }

      // If a number was pressed
      if (typedDigit != null) {
        // Ensure search bar has focus so user sees typing
        if (!widget.focusNode.hasFocus) {
          widget.focusNode.requestFocus();
        }

        _typedCode += typedDigit;
        widget.controller.text = _typedCode;
        widget.controller.selection = TextSelection.fromPosition(
          TextPosition(offset: _typedCode.length),
        );

        // Reset and start auto-search timer
        _typingTimer?.cancel();
        _typingTimer = Timer(const Duration(milliseconds: 1000), () async {
          if (_typedCode.isNotEmpty) {
            widget.onCodeTyped?.call(_typedCode);
            ...
            // Clear temporary code after search attempt
            _typedCode = '';
            widget.controller.clear();
          }
        });
      ...
      // Optional: Handle Backspace on TV remote to correct mistakes
      if (event.logicalKey == LogicalKeyboardKey.backspace && _typedCode.isNotEmpty) {
        _typedCode = _typedCode.substring(0, _typedCode.length - 1);
        widget.controller.text = _typedCode;
        widget.controller.selection = TextSelection.fromPosition(
          TextPosition(offset: _typedCode.length),
        );
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: TextField(
        controller: widget.controller,
        focusNode: widget.focusNode,
        readOnly: isTV, // Keeps virtual keyboard closed on TV
        showCursor: !isTV,
        enableInteractiveSelection: !isTV,
        keyboardType: TextInputType.number,
        inputFormatters: [
        ...
        onChanged: (value) {
          widget.onChanged?.call(value);
          // Only trigger this timer logic on Mobile (TV handles it via _handleKeyPress)
          if (!isTV) {
            _typingTimer?.cancel();
            _typingTimer = Timer(const Duration(milliseconds: 800), () {
              final code = value.trim();
